...
export class CmsService {
  ...
  async listArticles(publicOnly = false) {
    ...
    const result = await this.db.query(
      `SELECT
        ...
        CASE WHEN og.id IS NULL THEN NULL ELSE '/api/content/media/' || og.id END AS og_image_url,
        COALESCE((
       ...
       LEFT JOIN ${media} og ON og.id = a.og_image_media_id
       ${visibility}
  ...
  async createPreviewToken(articleId: string) {
    await this.findArticleById(articleId);
    const token = randomBytes(32).toString('base64url');
    const expiresAt = new Date(Date.now() + 15 * 60 * 1000);
    await this.db.query(
      `INSERT INTO ${this.db.table('cms_preview_tokens')}
        (token_hash, article_id, expires_at) VALUES ($1,$2,$3)`,
      [hashPreviewToken(token), articleId, expiresAt],
    );
    return { token, expiresAt: expiresAt.toISOString() };
  }

  async previewArticle(token: string) {
    const result = await this.db.query<{ article_id: string }>(
      `SELECT article_id FROM ${this.db.table('cms_preview_tokens')}
       WHERE token_hash=$1 AND expires_at > now()`,
      [hashPreviewToken(token)],
    );
    if (!result.rows[0]) throw new NotFoundException('Prévisualisation introuvable ou expirée.');
    ...
    await this.db.query(
      `INSERT INTO ${articles}
        (id, title, slug, excerpt, body_html, status, publish_at,
         category_id, cover_media_id, seo_title, seo_description, author,
         canonical_url, og_image_media_id, seo_index, gallery_media_ids)
  ...
  async updateArticle(id: string, dto: UpdateArticleDto) {
    ...
    if (dto.author !== undefined) add('author', dto.author.trim() || null);
    if (dto.canonicalUrl !== undefined) add('canonical_url', dto.canonicalUrl.trim() || null);
    if (dto.ogImageMediaId !== undefined) add('og_image_media_id', dto.ogImageMediaId || null);
    if (dto.seoIndex !== undefined) add('seo_index', dto.seoIndex);
    if (dto.galleryMediaIds !== undefined) add('gallery_media_ids', dto.galleryMediaIds);
    if (fields.length > 0) {
      values.push(id);
      await this.db.query(
  ...
  async listFaqs(publicOnly = false) {
    ...
    return result.rows.map((row) => ({
      ...
      answerHtml: cleanHtml(String(row.answer_html ?? '')),
...
function mapArticle(row: Record<string, unknown>) {
  return {
    ...
    slug: row.slug,
    excerpt: row.excerpt,
    bodyHtml: cleanHtml(String(row.body_html ?? '')),
    status: row.status,
