...
interface AdminDashboardProps {
  ...
  metrics: { products: number; stock: number; lowStock: number; value: number };
  updatedAt: string;
  environment: string;
...
export function AdminDashboard({
  ...
  const canUseEtl = canAccessModule(user.roles, 'etl');
  const canUseCms = canAccessModule(user.roles, 'cms');
  const canViewOperations = canAccessModule(user.roles, 'operations');
      ...
      <aside className={`sidebar ${sidebarOpen ? 'is-open' : ''}`}>
        ...
        <nav className="sidebar-nav" aria-label="Navigation principale">
          ...
          <a className="nav-link" href="#catalogue" onClick={closeSidebar}>
            <ShoppingBag size={19} />
            Catalogue
            <span className="nav-count">{metrics.products}</span>
          </a>
          ...
          {canUseEtl && <><a className="nav-link" href="#catalog-import" onClick={closeSidebar}>
            <Sheet size={19} />
            Import Excel
          </a>
          ...
          {canViewOperations && <a className="nav-link" href="#analytics" onClick={closeSidebar}>
            <BarChart3 size={19} />
            SEO & Marketing
          </a>}
          {canUseCms && <a className="nav-link" href="#cms" onClick={closeSidebar}>
            <FileText size={19} />
            CMS
          </a>}
          {canViewOperations && <a className="nav-link" href="/orders">Commandes</a>}
          {canViewOperations && <a className="nav-link" href="/transactions">Transactions</a>}
          {canViewOperations && <a className="nav-link" href="/invoices">Factures</a>}
          {canViewOperations && <a className="nav-link" href="/deliveries">Livraisons</a>}
          {canViewOperations && <a className="nav-link" href="/stock">Mouvements de stock</a>}
          {canViewOperations && <a className="nav-link" href="/products">Produits</a>}
          {canViewOperations && <a className="nav-link" href="/members">Membres</a>}
          {canViewOperations && <a className="nav-link" href="/payment-methods">Moyens de paiement</a>}
          {canViewOperations && <a className="nav-link" href="/delivery-zones">Zones de livraison</a>}
          {canUseCms && <a className="nav-link" href="/seo">SEO</a>}
          {canUseCms && <a className="nav-link" href="/social-publications">Publications sociales</a>}
          <a className="nav-link" href="/integrations">Intégrations</a>
          {user.roles.includes('ADMIN') && <a className="nav-link" href="/users">Utilisateurs</a>}
          {user.roles.includes('ADMIN') && <a className="nav-link" href="/audit-logs">Journal d’audit</a>}
        ...
        <main id="dashboard" className="dashboard-content">
          ...
          <section className="metrics-grid" aria-label="Indicateurs catalogue">
            <MetricCard
              ...
              value={number.format(metrics.products)}
          ...
          {canViewOperations && <OperationsPanel />}

          {canUseEtl && <CatalogImportPanel />}
          {canUseEtl && <PartnerImportPanel />}
          {canViewOperations && <AnalyticsPanel />}
          {canUseCms && <CmsPanel />}
