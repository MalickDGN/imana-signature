...
export function CatalogImportPanel() {
  ...
  const dryRun = async () => {
    ...
    try {
      const response = await fetch(`/api/catalog-import/${encodeURIComponent(result.batchId)}/dry-run`, {
  ...
  const retryFailed = async () => {
    ...
    try { const response = await fetch(`/api/catalog-import/${encodeURIComponent(result.batchId)}/retry`, { method: 'POST' }); const payload = await readJson(response); if (!response.ok) throw new Error(getMessage(payload)); setResult(payload as ImportResult); }
    ...
    <section className="panel import-panel" id="catalog-import">
      ...
      {!result && (
        ...
        >
          ...
          <strong>
            {file ? file.name : 'Déposez votre fichier catalogue ici'}
          </strong>
          <p>
            {file
