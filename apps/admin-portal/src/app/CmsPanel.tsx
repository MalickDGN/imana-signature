...
function ArticleEditor({ article, categories, tags, media, onClose, onSaved }: {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      canonicalUrl: form.get('canonicalUrl') || undefined,
      coverMediaId: form.get('coverMediaId') || undefined,
      ogImageMediaId: form.get('ogImageMediaId') || undefined,
      galleryMediaIds: form.getAll('galleryMediaIds'),
  ...
  async function previewOnSite() {
    if (!article) return;
    setError('');
    const response = await fetch(`/api/admin/cms/articles/${article.id}/preview-token`, { method: 'POST' });
    const result = await response.json() as { token?: string; message?: string };
    if (!response.ok || !result.token) return setError(result.message ?? 'Prévisualisation impossible.');
    const storefront = process.env.NEXT_PUBLIC_STOREFRONT_URL ?? 'http://localhost:3000';
    window.open(`${storefront}/blog/preview/${encodeURIComponent(result.token)}`, '_blank', 'noopener,noreferrer');
  }
  ...
  return (
    <form className="editor-panel panel" onSubmit={submit}>
      ...
      <div className="editor-grid">
        ...
        <label>Auteur<input name="author" maxLength={160} defaultValue={article?.author} /></label>
        <label>Statut<select name="status" defaultValue={article?.status ?? 'draft'}><option value="draft">Brouillon</option><option value="in_review">En validation</option><option value="scheduled">Planifié</option><option value="published">Publié</option><option value="unpublished">Dépublié</option><option value="archived">Archivé</option></select></label>
        ...
        <label>Image principale<select name="coverMediaId" defaultValue={article?.coverMediaId ?? ''}><option value="">Aucune</option>{media.filter((item) => item.mimeType.startsWith('image/')).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label>Image Open Graph<select name="ogImageMediaId" defaultValue={article?.ogImageMediaId ?? ''}><option value="">Image principale</option>{media.filter((item) => item.mimeType.startsWith('image/')).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <fieldset className="field-wide tag-selector"><legend>Galerie</legend>{media.filter((item) => item.mimeType.startsWith('image/')).map((item) => <label key={item.id}><input type="checkbox" name="galleryMediaIds" value={item.id} defaultChecked={article?.galleryMediaIds?.includes(item.id)} /> {item.name}</label>)}</fieldset>
        ...
        <label className="field-wide">URL canonique<input name="canonicalUrl" type="url" maxLength={500} defaultValue={article?.canonicalUrl} /></label>
        <label className="field-wide"><input name="seoIndex" type="checkbox" defaultChecked={article?.seoIndex ?? true} /> Autoriser l’indexation</label>
      </div>
      ...
      <div className="editor-actions">
        ...
        {article && <button className="secondary-action" type="button" onClick={() => void previewOnSite()}><Eye size={16} /> Prévisualiser sur le site</button>}
