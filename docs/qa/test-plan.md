...
### Régression de la migration React

Exécuter `pnpm run test:frontend` (ou `pnpm.cmd run test:frontend` sous
PowerShell) pour lancer Next.js sur le port 3100 et vérifier Chromium sur
ordinateur et Pixel 5. Le port doit être libre ou servir ce même frontend.

La suite `tests/e2e/storefront-alignment.spec.js` couvre les quatre routes
publiques et leurs alias `.html`, l'hydratation par ouverture/fermeture du
panier, les styles et médias, l'absence de scripts historiques, la navigation,
le diaporama, les filtres d'URL et la persistance/suppression du panier.
Elle vérifie aussi le catalogue vide, une erreur API suivie d'une reprise et
l'impossibilité d'ajouter un produit épuisé.

Les réponses catalogue, session, articles et analytics sont simulées uniquement dans
les tests. Cette suite ne remplace pas la recette Odoo et paiement sur un
environnement connecté. Les captures et traces d'échec se trouvent dans
`test-results/`. Les tests Express historiques restent lancés séparément par
`pnpm run test:e2e`.
